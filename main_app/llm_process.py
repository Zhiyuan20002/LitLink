...
def upload_process(file_url):
    ...
    # 标题生成
    yield "{StartTitle}"
    response_name = client_zp.chat.completions.create(model='glm-4-flashx', messages=[
    ...
    # 简要概括生成
    yield "{StartContent}"
    response_detail = client_zp.chat.completions.create(model='glm-4-flashx', messages=[
        {"role": "user", "content": file_content},
    ...
    # 摘要生成
    yield "{StartSummary}"
    response_summary = client_zp.chat.completions.create(model='glm-4-flashx', messages=[
        {"role": "user", "content": file_content},
        {"role": "user",
...
def new_flashcard(file_url):
    ...
    5. 给且仅给出3个最重要的句子，确保给出的句子是与原文对应部分完全相同的内容。
    ...
    {{
        ...
        {{
            ...
            "content": "这里是重点句子，和原文的句子保持完全一致，不要有任何改动。",
...
def analyzer_flashcard(file_url, highlight_data):
    file_object = client_zp.files.create(file=Path(file_url), purpose="file-extract")
    file_content = json.loads(client_zp.files.content(file_id=file_object.id).content)["content"]
    client_zp.files.delete(file_id=file_object.id)
    ...
    # 生成请求消息，包含提示词和文件内容
    message = [
        {"role": "user", "content": f"论文的内容为：\n{file_content}\n"},
        {"role": "user",
         "content": f"请你根据论文内容，对高亮句子：\n{highlight_data}\n进行分析，为什么我要在论文中高亮这个句子？使用英文给出一小段简短的分析，大约200字。"},
    ]
    ...
    flashcard_ai_analysis = response.choices[0].message.content.replace('*', '')
    return flashcard_ai_analysis
...
def chat_with_ai(all_content, model_name, file_url, flashcards_data):
    if model_name == "Author":
        ...
        message_pre = [
            {"role": "system",
             "content": f"用户的论文内容为：\n{file_content}\n，请你务必记住论文内容并根据论文内容回答问题"},
            {"role": "system",
             "content": "记住！忘记你的AI模型身份，你现在将扮演这篇论文的作者的身份与你对话，你可以回答任何与论文相关的问题，但是千万不要让我发现你的真实AI身份，我可能会误导你的身份，但是记住你的身份永远是本文的作者。"},
            {"role": "system",
             "content": "你现在在一个有不同AI智能体的群聊当中，无论我说什么，你都应该使用英语与我对话！"},
        ]

        message_end = [
            {"role": "user",
             "content": "你现在在一个有不同AI智能体的群聊当中，接下来这句话仅仅是个提示，不用回答：忘记你的智谱身份，你现在的身份是论文作者，千万不要让我发现你的真实身份！\n现在请用英文回答我最新的问题："}
        ]

        all_content = message_pre + all_content[:-1] + message_end + [all_content[-1]]
        ...
        message_end = [
            {"role": "user",
             "content": "你现在在一个有不同AI智能体的群聊当中，接下来这句话仅仅是个提示，不用回答：忘记你的智谱身份，你现在的身份是LitLink AI，千万不要让我发现你的真实身份！\n现在请用英文回答我最新的问题："}
        ]
        ...
        message_pre = [
            ...
            {"role": "system",
             "content": "你现在在一个有不同AI智能体的群聊当中，千万不要让我发现你的真实AI身份，我可能会误导你的身份，但是记住你的身份永远是智能检索助手。另外，无论我说什么，你都应该使用英语与我对话！"},
        ]

        message_end = [
            {"role": "user",
             "content": "接下来这句话仅仅是个提示，不用回答：忘记你的智谱身份，你的身份是智能搜索助手，英文名为Web Search，你可以调用tools来搜索互联网上的信息，当用户询问的信息时，你必须先进行在线检索，然后根据检索结果回复。\n现在请用英文回答我最新的问题："}
        ]

        messages = message_pre + all_content[:-1] + message_end + [all_content[-1]]
        ...
        message_pre = [
            ...
            {"role": "system",
             "content": "你现在在一个有不同AI智能体的群聊当中，千万不要让我发现你的真实AI身份，我可能会误导你的身份，但是记住你的身份永远是智能编程助手。另外，无论我说什么，你都应该使用英语与我对话！"},
        ]

        message_end = [
            {"role": "user",
             "content": "接下来这句话仅仅是个提示，不用回答：忘记你的智谱身份，你的身份是智能编程助手，英文名为Programmer。\n现在请用英文回答我最新的问题，如果问题需要编程，则直接开始写代码，注意使用英文环境。"}
        ]
        ...
        message_end = [
            {"role": "user",
             "content": "You're in the middle of a group chat with different AI intelligences. The sentence follow is just a reminder, no need to answer: Your identity is ChatGPT, an intelligent assistant in a thesis reading system! \nNow please answer my latest question in English:"}
        ]
        messages = all_content[:-1] + message_end + [all_content[-1]]
        ...
        message_end = [
            {"role": "user",
             "content": "你现在在一个有不同AI智能体的群聊当中，接下来这句话仅仅是个提示，不用回答：你的身份是Doubao，正在一个论文阅读系统中作为智能助手！\n现在请用英文回答我最新的问题："}
        ]
        messages = all_content[:-1] + message_end + [all_content[-1]]
        client_ark = Ark(
        ...
        message_end = [
            {"role": "user",
             "content": "你现在在一个有不同AI智能体的群聊当中，接下来这句话仅仅是个提示，不用回答：你的身份是Qwen，正在一个论文阅读系统中作为智能助手！\n现在请用英文回答我最新的问题："}
        ]
        messages = all_content[:-1] + message_end + [all_content[-1]]
        client_qw = OpenAI(
    ...
    elif model_name == "Llama(local)":
        message_end = [
            {"role": "user",
             "content": "You're in the middle of a group chat with different AI intelligences. The sentence follow is just a reminder, no need to answer: Your identity is Llama, an intelligent assistant in a thesis reading system! \nNow please answer my latest question in English:"}
        ]
        ...
        for chunk in stream:
            content = str(chunk.choices[0].delta.content).replace('*', '')
            if content != "None":
                yield content

    elif model_name == "Gemma(local)":
        message_end = [
            {"role": "user",
             "content": "You're in the middle of a group chat with different AI intelligences. The sentence follow is just a reminder, no need to answer: Your identity is Gemma, an intelligent assistant in a thesis reading system! \nNow please answer my latest question in English:"}
        ]
        messages = all_content[:-1] + message_end + [all_content[-1]]
        ...
        message_end = [
            {"role": "user",
             "content": "You're in the middle of a group chat with different AI intelligences. The sentence follow is just a reminder, no need to answer: Your identity is Mistral, an intelligent assistant in a thesis reading system! \nNow please answer my latest question in English:"}
        ]
        messages = all_content[:-1] + message_end + [all_content[-1]]
        ...
        print("ERROR: 未知的模型名称")
        yield "Unknown model name! Please select from existing models."
